...
const RegionAdminInstructionsAdd = () => {
  ...
  const { data: districts, refetch: districtRefetch } = getQuery<any>(
    'common/districts',
    undefined,
  ...
  const handleSubmit = (values) => {
    ...
    const data = {
      ...
      to_district: values.to_district.map((item) => item.id),
      to_employee:
        values.to_role.includes('4' || '5') && values.to_district.length == 1
          ? values.to_employee.map((item) => item.id)
          : null,
      required_to_district:
        values.to_employee.length == 0 ? values.required_to_district.map((item) => item.id) : null,
      required_to_employee:
        (values.to_district.length > 0
          ? values.to_district.length == 1 && values.to_employee.length > 0
                  ...
                  >
                    ...
                    {values.to_employee.length == 0 && (
                      <FormikSelect
                        ...
                        options={
                          values.to_district.length > 0 ? values.to_district : districts?.results
                        }
                        getOptionLabel={(opt) => opt.name}
                        disabled={view || isPending || isEditing || isEdit}
                      />
                    ...
                    {(values.to_district.length > 0
                      ...
                      values.to_role.includes('4' || '5') && (
                        <FormikSelect
                          ...
                          options={
                            values.to_district.length > 0 ? values.to_employee : mosques?.results
                          }
                          getOptionLabel={(opt) =>
                            opt.district || opt.district__name
                              ? opt.name + ' | ' + (opt.district?.name || opt.district__name)
                          ...
                          placeholder={values.to_district.length > 0 ? 'Tanlang' : 'Yozib qidiring'}
                          disabled={view || isPending || isEditing || isEdit}
                          onInputChange={(_, newInputValue) => {
                            if (values.to_district.length == 0) {
...
export default RegionAdminInstructionsAdd
